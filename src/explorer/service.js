...
function sendGet(url, params) {
  //if(url == '/NRC20-txs' || url == '/NRC20-latest')
...
export default {
  ...
  getNRC20Txs(params) {
    return authGet('/NRC20-txs', { params }).then(res => {
      return res.data
    })
  },
  getNRC20TxsLatest(params) {
    return authGet('/NRC20-latest', { params }).then(res => {
      return res.data
    })
  },
  ...
  async search(input, tryEth = true) {
    ...
    return await authGet('/search', { params: { input } }).then(res => {
      ...
      if (input.length === 66 && tryEth) {
        ...
        console.log('Converted ETH tx hash to Nordic Energy, searching...')
