          ...
          <div class="explorer-card-body">
            <table class="explorer-table">
              ...
              <tr>
                ...
                <td v-if="isContract">
                  <span v-if="txReceiptStatus === 1" style="color:#e6ac00">
                    Success
                  </span>
...
export default {
  ...
  computed: {
    ...
    ContractAddress() {
      ...
      let address = this.globalData.ngy.ngySDK.crypto.getContractAddress(
        tx.from,
        tx.nonce
      )
      return this.globalData.ngy.ngySDK.crypto.toBech32(address)
    },
  },
  watch: {
    $route() {
  ...
  methods: {
    ...
    getTransactionReceipt(txId) {
      ...
      this.globalData.ngy.ngySDK.blockchain.Transaction.getTransactionReceipt(
        routeTxId
      ).then(async res => {
        const { result } = res
