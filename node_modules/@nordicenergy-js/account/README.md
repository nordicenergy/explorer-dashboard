...
`@nordicenergy-js/account` is dealing with account related features.
...
The `Account` class is basic instance that contains most features mentiNetd above.
...
```typescript
    ...
    const myAccountWitNgyPrivateKeyUsingStatic = Account.add(myPrivateKey)

    console.log({ myAccountWitNgyPrivateKey, myAccountWitNgyPrivateKeyUsingStatic })
...
```typescript
    ...
    async function importKeyStoreFileAndDecrypt(keyStoreFile, password) {
        ...
        // see if the privatekey is equal to unencrypted Net?
        console.log(
...
```typescript
    ...
    console.log(myAccountWitNgyPrivateKey.address)
    // Account.bech32Address is bech32 format address, in NordicEnergy, it's `Net1` prefixed
    console.log(myAccountWitNgyPrivateKey.bech32Address)
    // Account.bech32TestNetAddress is bech32 format address, in NordicEnergy, it's `tNet1` prefixed, used in testnet
    console.log(myAccountWitNgyPrivateKey.bech32TestNetAddress)
    // Account.checksumAddress is checksumed address from base16
    console.log(myAccountWitNgyPrivateKey.checksumAddress)
...
```typescript
    ...
    // import Transaction class from '@nordicenergy-js/transaction'
    import {Transaction} from '@nordicenergy-js/transaction'
    
     // Messenger is optional, by default, we have a defaultMessenger 
    // If you like to change, you will import related package here.
    import { HttpProvider, Messenger } from '@nordicenergy-js/network';
    import { ChainType, ChainID, Unit } from '@nordicenergy-js/utils';

    // create a custom messenger
    const customMessenger = new Messenger(
        new HttpProvider('http://localhost:9500'),
        ChainType.NordicEnergy, // if you are connected to NordicEnergy's blockchain
        ChainID.NgyLocal, // check if the chainId is correct
    )
    // suppose we have an account
    const myPrivateKey = '0xe19d05c5452598e24caad4a0d85a49146f7be089515c905ae6a19e8a578a6930'
    ...
    async function signTheTxn() {
        ...
        const signedTxn = await myAccountWitNgyPrivateKey.signTransaction(txn, false, 'rlp', 'latest');
